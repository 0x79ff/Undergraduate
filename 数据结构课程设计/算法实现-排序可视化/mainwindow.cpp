...
{
    ui->setupUi(this);
    this->setWindowTitle(QString("排序算法可视化"));
    this->setWindowIcon(QIcon(":/iconsrc/icon.ico"));

    // 添加需要的字体
    int fontId = QFontDatabase::addApplicationFont(QStringLiteral(":/fontsrc/font/hgy3.ttf"));
    ...
    // 给下拉框添加内容
        ...
        ui->comboBox_sortingalgorithm->addItem(tr("插入排序     O(n^2)     稳定"));         //  Index 0
        ui->comboBox_sortingalgorithm->addItem(tr("折半插入     O(n^2)     稳定"));         //  Index 1
        ui->comboBox_sortingalgorithm->addItem(tr("希尔排序     O(n^1.3)   不稳定"));         //  Index 2
        ui->comboBox_sortingalgorithm->addItem(tr("冒泡排序     O(n^2)     稳定"));         //  Index 3
        ui->comboBox_sortingalgorithm->addItem(tr("快速排序     O(nlogn)   不稳定"));          //  Index 4
        ui->comboBox_sortingalgorithm->addItem(tr("选择排序     O(n^2)     不稳定"));          //  Index 5
    ...
    ui->ShowSortingVisulizaion->setStyleSheet("border-color: black;""border-width: 1px;" "border-style: solid;");
...
void MainWindow::on_btn_dataGenerate_clicked()
{
    ...
    if (bOk && dataNum >= 0)
    {
        ...
        dataGenerating = 1;         // 修改状态，防止生成过程中再次点击生成数据按钮
        ...
        // 设置可以调整窗口大小（每次开始排序时禁止改变窗口大小，点击生成数据会停止上次排序，数据添加完毕后恢复窗口大小设置）
...
void MainWindow::on_btn_sortingStart_clicked()
{
    ...
    if(!datahaved)
    {
        QMessageBox::warning(this, "提示", "请先随机生成数据！");
        return;
    }


    // 状态栏显示排序算法   不直接调用comboBox的currentText，避免时间复杂度和稳定性间距太大，阅读不舒服
        if(algorithmSelected == 0) sbShow(1, "插入排序");
        else if(algorithmSelected == 1) sbShow(1, "折半插入");
        else if(algorithmSelected == 2) sbShow(1, "希尔排序");
        else if(algorithmSelected == 3) sbShow(1, "冒泡排序");
        else if(algorithmSelected == 4) sbShow(1, "快速排序");
        else if(algorithmSelected == 5) sbShow(1, "选择排序");
...
void MainWindow::updateColumns(int a, int b, int *newHeights) // newHeights与numbers是一个数组。为了含义清晰，所以重新传回来作为newHeights
{
    ...
    //用红色指示当前交换的元素
    columns[a].setStyleSheet("background-color: rgb(27,167,132);"
                             "border-color: rgb(27,167,132);"
                             "border-width: 1px;"
                             "border-style: solid;");
    columns[b].setStyleSheet("background-color:rgb(27,167,132);"
                             "border-color: rgb(27,167,132);"
                             "border-width: 1px;"
                             "border-style: solid;");
...
void MainWindow::updateOneColumn(int c, bool resetPrev, int *newHeights) // newHeights与numbers是一个数组。为了含义清晰，所以重新传回来作为newHeights
{
    ...
    //指示当前更新的元素
    columns[c].setStyleSheet("background-color: rgb(27,167,132);"
                             "border-color: rgb(27,167,132);"
                             "border-width: 1px;"
                             "border-style: solid;");
...
void MainWindow::highLightColumns(int a, int b)
{
    ...
    // 指示当前比较的元素
    columns[a].setStyleSheet("background-color: rgb(43, 115, 175);"
                             "border-color: rgb(43, 115, 175);"
                             "border-width: 1px;"
                             "border-style: solid;");
    columns[b].setStyleSheet("background-color: rgb(43, 115, 175);"
                             "border-color: rgb(43, 115, 175);"
                             "border-width: 1px;"
                             "border-style: solid;");
...
void MainWindow::highLightBase(int base)
{
    ...
    // 指示当前基准值
    columns[base].setStyleSheet("background-color: rgb(238, 63, 77);"
                                "border-color:  rgb(238, 63, 77);"
                                "border-width: 1px;"
                                "border-style: solid;");
...
//    this->show();
    ...
    for(int i = 0; i < dataNum; i++)
    {
        ...
        columns[i].setStyleSheet("background-color: rgb(0,255,255);"
