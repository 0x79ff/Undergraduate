...
int write_compressedfile(char *orifile)
{
    ...
    compress_file = fopen(compressedfile,"w");
...
int write_decompressedfile(char *orifile,int root)
{
    ...
    decompress_file = fopen(decompressedfile,"w");
    if(decompress_file == NULL) return FALSE;
    compress_file = fopen(compressedfile,"r");
    ...
    while ((ch = fgetc(compress_file)) != EOF) {
        for(int i=7;i>=0;i--){
            if((char)(ch & (1<<i))!=0) ListInsert_Sq(s,s.length+1,'1');
